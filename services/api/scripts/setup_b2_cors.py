...
existing CORS rules are preserved and one rule (ID `kiss-icp-direct-upload`) is
...
RULE_ID = "kiss-icp-direct-upload"
...
def _client():
    ...
    # CORS calls sign more reliably with an explicit region. The region comes
    # from B2_REGION (the single source of truth), never a hardcoded literal.
    return boto3.client(
        "s3",
        ...
        config=Config(signature_version="s3v4", user_agent_extra="b2ai-kiss-icp-lidar-archive"),
    )
